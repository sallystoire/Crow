  ...
  try {
    ...
    for (const ep of editPacksRows) {
      store.editPacks.push({
        packId: ep.grantorRoleId,
        grantorRoleId: ep.grantorRoleId,
        allowedRoleIds: ep.allowedRoleIds as string[],
      });
    }
    ...
    for (const alr of allowLinkRows) {
      if (!store.allowLinkRoles.has(alr.channelId)) {
        store.allowLinkRoles.set(alr.channelId, new Set());
      }
    ...
    if (guildConfig[0]) {
      ...
      if (cfg.antiDecoLimit) {
        store.antiDecoLimit = parseInt(cfg.antiDecoLimit);
      }
    }
...
export async function dbAddBan(guildId: string, entry: BanEntry): Promise<void> {
  if (!db) return;
  await db.insert(botBanEntries).values({
    guildId,
    userId: entry.userId,
    ...
    target: [botBanEntries.guildId, botBanEntries.userId],
    set: { username: entry.username, reason: entry.reason, bannedAt: entry.bannedAt },
  });
}

export async function dbRemoveBan(guildId: string, userId: string): Promise<void> {
  if (!db) return;
  await db.delete(botBanEntries).where(
    and(eq(botBanEntries.guildId, guildId), eq(botBanEntries.userId, userId))
  );
}
...
export async function dbAddBlacklist(guildId: string, entry: BlacklistEntry): Promise<void> {
  if (!db) return;
  await db.insert(botBlacklistEntries).values({
    guildId,
    userId: entry.userId,
    username: entry.username,
    reason: entry.reason,
    ...
    target: [botBlacklistEntries.guildId, botBlacklistEntries.userId],
    set: { username: entry.username, reason: entry.reason },
  });
}

export async function dbRemoveBlacklist(guildId: string, userId: string): Promise<void> {
  if (!db) return;
  await db.delete(botBlacklistEntries).where(
    and(eq(botBlacklistEntries.guildId, guildId), eq(botBlacklistEntries.userId, userId))
  );
}
...
export async function dbAddMute(guildId: string, entry: MuteEntry): Promise<void> {
  if (!db) return;
  await db.insert(botMuteEntries).values({
    guildId,
    userId: entry.userId,
    username: entry.username,
    reason: entry.reason,
    ...
    target: [botMuteEntries.guildId, botMuteEntries.userId],
    set: { reason: entry.reason, endsAt: entry.endsAt ?? null },
  });
}

export async function dbRemoveMute(guildId: string, userId: string): Promise<void> {
  if (!db) return;
  await db.delete(botMuteEntries).where(
    and(eq(botMuteEntries.guildId, guildId), eq(botMuteEntries.userId, userId))
  );
}
...
export async function dbAddOwner(guildId: string, userId: string): Promise<void> {
  if (!db) return;
  await db.insert(botOwnerList).values({ guildId, userId })
    .onConflictDoNothing();
}

export async function dbRemoveOwner(guildId: string, userId: string): Promise<void> {
  if (!db) return;
  await db.delete(botOwnerList).where(
    and(eq(botOwnerList.guildId, guildId), eq(botOwnerList.userId, userId))
  );
}
...
export async function dbAddWl(guildId: string, userId: string): Promise<void> {
  if (!db) return;
  await db.insert(botWlList).values({ guildId, userId })
    .onConflictDoNothing();
}

export async function dbRemoveWl(guildId: string, userId: string): Promise<void> {
  if (!db) return;
  await db.delete(botWlList).where(
    and(eq(botWlList.guildId, guildId), eq(botWlList.userId, userId))
  );
}
...
export async function dbAddHideMe(guildId: string, userId: string): Promise<void> {
  if (!db) return;
  await db.insert(botHideMeList).values({ guildId, userId })
    .onConflictDoNothing();
}

export async function dbRemoveHideMe(guildId: string, userId: string): Promise<void> {
  if (!db) return;
  await db.delete(botHideMeList).where(
    and(eq(botHideMeList.guildId, guildId), eq(botHideMeList.userId, userId))
  );
}
...
export async function dbAddAntiMove(guildId: string, userId: string): Promise<void> {
  if (!db) return;
  await db.insert(botAntiMoveList).values({ guildId, userId })
    .onConflictDoNothing();
}

export async function dbRemoveAntiMove(guildId: string, userId: string): Promise<void> {
  if (!db) return;
  await db.delete(botAntiMoveList).where(
    and(eq(botAntiMoveList.guildId, guildId), eq(botAntiMoveList.userId, userId))
  );
}
...
export async function dbRemoveAutomate(guildId: string, trigger: string): Promise<void> {
  if (!db) return;
  await db.delete(botAutomateEntries).where(
    and(eq(botAutomateEntries.guildId, guildId), eq(botAutomateEntries.trigger, trigger))
  );
}
...
export async function dbSaveCustomPerms(guildId: string, roleId: string, perms: Set<string>): Promise<void> {
  if (!db) return;
  await db.insert(botCustomPerms).values({
    guildId,
    roleId,
    perms: Array.from(perms),
  }).onConflictDoUpdate({
    target: [botCustomPerms.guildId, botCustomPerms.roleId],
    set: { perms: Array.from(perms) },
  });
}
...
export async function dbSaveEditPack(guildId: string, grantorRoleId: string, allowedRoleIds: string[]): Promise<void> {
  ...
  await db.insert(botEditPacks).values({ guildId, grantorRoleId, allowedRoleIds })
    .onConflictDoUpdate({
      target: [botEditPacks.guildId, botEditPacks.grantorRoleId],
      set: { allowedRoleIds },
    });
}
...
export async function dbRemoveEditPack(guildId: string, grantorRoleId: string): Promise<void> {
  if (!db) return;
  await db.delete(botEditPacks).where(
    and(eq(botEditPacks.guildId, guildId), eq(botEditPacks.grantorRoleId, grantorRoleId))
  );
}
...
export async function dbRemoveSecureRole(guildId: string, roleId: string): Promise<void> {
  if (!db) return;
  await db.delete(botSecureRoles).where(
    and(eq(botSecureRoles.guildId, guildId), eq(botSecureRoles.roleId, roleId))
  );
}
...
export async function dbAddAntiLinkChannel(guildId: string, channelId: string): Promise<void> {
  if (!db) return;
  await db.insert(botAntiLinkChannels).values({ guildId, channelId })
    .onConflictDoNothing();
}

export async function dbAddAllowLinkRole(guildId: string, channelId: string, roleId: string): Promise<void> {
  if (!db) return;
  await db.insert(botAllowLinkRoles).values({ guildId, channelId, roleId })
    .onConflictDoNothing();
}
...
export async function dbSaveGuildConfig(guildId: string, store: GuildStore): Promise<void> {
  if (!db) return;
  await db.insert(botGuildConfigs).values({
    guildId,
    ...
    muteLevels: store.muteConfig.levels,
