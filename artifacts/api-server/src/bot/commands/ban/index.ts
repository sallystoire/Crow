...
export async function handleBan(msg: Message, args: string[]): Promise<void> {
  ...
  if (!target) {
    await msg.reply({ embeds: [errorEmbed("Mentionne un membre à bannir.")] });
    return;
  }
  if (target.id === msg.author.id) {
    await msg.reply({ embeds: [errorEmbed("Tu ne peux pas te bannir toi-même.")] });
    return;
  }
  ...
  try {
    ...
    await msg.reply({
      embeds: [successEmbed(`Vous avez banni **${target.user.tag}** du serveur pour **${reason}**`)],
    });
  } catch {
...
export async function handleUnban(msg: Message, args: string[]): Promise<void> {
  ...
  if (!userId) {
    await msg.reply({ embeds: [errorEmbed("Fournis l'ID ou la mention de l'utilisateur.")] });
    return;
  }
...
export async function handleKick(msg: Message, args: string[]): Promise<void> {
  ...
  const target = msg.mentions.members?.first();
  if (!target) {
    await msg.reply({ embeds: [errorEmbed("Mentionne un membre à kick.")] });
    return;
  }
  if (target.id === msg.author.id) {
    await msg.reply({ embeds: [errorEmbed("Tu ne peux pas te kick toi-même.")] });
    return;
  }
  ...
  try {
    ...
    await msg.reply({
      embeds: [successEmbed(`Vous avez kick **${target.user.tag}** du serveur pour **${reason}**`)],
    });
  } catch {
...
export async function handleBlacklist(msg: Message, args: string[]): Promise<void> {
  ...
  const target = msg.mentions.members?.first();
  if (!target) {
    await msg.reply({ embeds: [errorEmbed("Mentionne un membre à blacklister.")] });
    return;
  }
  if (target.id === msg.author.id) {
    await msg.reply({ embeds: [errorEmbed("Tu ne peux pas te blacklister toi-même.")] });
    return;
  }
  ...
  try {
    ...
    await msg.reply({
      embeds: [successEmbed(`Vous avez blacklist **${target.user.tag}** du serveur pour **${reason}**`)],
    });
  } catch {
