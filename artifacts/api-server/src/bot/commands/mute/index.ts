...
export async function handleSetupMuteParam(msg: Message, args: string[]): Promise<void> {
  ...
  if (subCmd === "channel") {
    ...
    if (!channel) { await msg.reply({ embeds: [errorEmbed("Mentionne un salon.")] }); return; }
  ...
  if (subCmd === "maxtime") {
    ...
    if (isNaN(minutes) || minutes < 1) { await msg.reply({ embeds: [errorEmbed("Durée invalide.")] }); return; }
  ...
  if (subCmd === "level") {
    ...
    if (isNaN(level) || roles.length === 0) { await msg.reply({ embeds: [errorEmbed("Format: `.setupmute level N @role1 @role2`")] }); return; }
    const existing = store.muteConfig.levels.find((l) => l.level === level);
    if (existing) {
      existing.roleIds = roles.map((r) => r.id);
    } else {
      store.muteConfig.levels.push({ level, roleIds: roles.map((r) => r.id), canTempmute: true, canUnmute: level >= 2 });
    }
    await dbSaveGuildConfig(msg.guild.id, store);
    await msg.reply({ embeds: [successEmbed(`Niveau ${level} configuré: ${roles.map((r) => `<@&${r.id}>`).join(", ")}`)] });
    return;
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      "`.setupmute maxtime minutes` — définir la durée max\n" +
      "`.setupmute level N @role1 @role2` — définir les niveaux\n\n" +
      `**Config actuelle:**\n` +
      ...
      `Niveaux: **${store.muteConfig.levels.length}**`
    ).setTimestamp();
...
export async function handleTempMute(msg: Message, args: string[]): Promise<void> {
  ...
  const store = getGuildStore(msg.guild.id);
  if (!store.muteConfig.muteRoleId) { await msg.reply({ embeds: [errorEmbed("Configure d'abord un rôle mute avec `.setupmute role @role`.")] }); return; }

  const duration = parseInt(args[1] ?? "10", 10);
  const reason = args.slice(2).join(" ") || "Aucune raison";
  if (isNaN(duration) || duration < 1) { await msg.reply({ embeds: [errorEmbed("Durée invalide (en minutes).")] }); return; }

  const maxDuration = store.muteConfig.maxDurationMinutes;
  if (!isOwner(msg.member!) && duration > maxDuration) { await msg.reply({ embeds: [errorEmbed(`La durée max autorisée est ${maxDuration} minutes.`)] }); return; }
  ...
  try {
    ...
    await msg.reply({ embeds: [successEmbed(`🔇 **${target.user.tag}** muté pour **${duration} minutes** — *${reason}*`)] });
  } catch {
