...
import { dbAddAntiLinkChannel, dbAddAllowLinkRole } from "../../db.js";
...
export async function handleAntiLink(msg: Message, args: string[]): Promise<void> {
  ...
  await dbAddAntiLinkChannel(msg.guild.id, channel.id);
...
export async function handleAllowLink(msg: Message): Promise<void> {
  ...
  const channel = msg.mentions.channels.first() ?? msg.channel;
  if (!role) { await msg.reply({ embeds: [errorEmbed("Mentionne un rôle à autoriser.")] }); return; }
  const store = getGuildStore(msg.guild.id);
  if (!store.allowLinkRoles.has(channel.id)) store.allowLinkRoles.set(channel.id, new Set());
  store.allowLinkRoles.get(channel.id)!.add(role.id);
  await dbAddAllowLinkRole(msg.guild.id, channel.id, role.id);
  await msg.reply({ embeds: [successEmbed(`✅ <@&${role.id}> peut envoyer des liens dans <#${channel.id}>.`)] });
}
