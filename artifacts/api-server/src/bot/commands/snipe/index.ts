...
export async function handleSnipe(msg: Message, args: string[]): Promise<void> {
  ...
  if (mentionedUser) {
    ...
    const visible = userSnipes
      .filter((s) => !store.hideMeList.has(s.authorId))
      .slice(0, 5);

    if (visible.length === 0) {
      await msg.reply({ embeds: [errorEmbed(`Aucun message supprimé trouvé pour <@${mentionedUser.id}>.`)] });
      return;
    }
    ...
    for (let i = 0; i < visible.length; i++) {
      const s = visible[i]!;
      embed.addFields({
        name: `Message #${i + 1} — ${s.deletedAt.toLocaleTimeString("fr-FR")}`,
        value: s.content || "*[Contenu vide]*",
      });
    }
  ...
  if (isNaN(count) || count < 1) {
    await msg.reply({ embeds: [errorEmbed("Fournis un nombre valide (max 3) ou mentionne un utilisateur.")] });
    return;
  }
  ...
  if (channelSnipes.length === 0) {
    await msg.reply({ embeds: [errorEmbed("Aucun message supprimé récemment dans ce salon.")] });
    return;
  }

  const embed = new EmbedBuilder()
    .setColor(0x9b59b6)
    .setTitle(`🔍 ${channelSnipes.length} dernier(s) message(s) supprimé(s)`)
    .setTimestamp();

  for (let i = 0; i < channelSnipes.length; i++) {
    const s = channelSnipes[i]!;
    embed.addFields({
      name: `**${s.authorUsername}** — ${s.deletedAt.toLocaleTimeString("fr-FR")}`,
      value: s.content || "*[Contenu vide]*",
    });
  }
